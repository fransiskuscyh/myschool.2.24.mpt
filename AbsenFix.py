...
class absen(ct.CTkFrame):
    ...
    def ui(self, masterss):
        ...
        self.button_cari = ct.CTkButton(master=masterss, height=80, width=150, text='Cari', fg_color='#0060ff',
                                        font=('montserrat', 17), command=lambda: self.execute_cari())
        self.button_ubah = ct.CTkButton(master=masterss, height=80, width=150, text='Simpan', fg_color='#ff5f00',
                                        font=('montserrat', 17), command=lambda : self.update_absen())
        ...
        sql1 = 'SELECT mataPelajaran FROM `mata_pelajaran`'
        self.curr.execute(sql1)
        matpel = self.curr.fetchall()
        sql2 = 'SELECT nama_kelas FROM `kelas`'
         #   self.list_matkul = x
        self.curr.execute(sql2)
        kelaslist = self.curr.fetchall()
        self.list_kelas = [t[0] for t in kelaslist]
        self.list_matkul = [t[0] for t in matpel]
    ...
    def item_selected(event, self):
        for selected_item in self.table.selection():
            ...
            record = item['values']
            # show a message
            self.data_show(title='Information', message=','.join(record))
